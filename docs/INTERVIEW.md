...
2. **Architecture (45 seconds):** Browser → Flask routes → checkout service → SQLite. Why Python and why no separate frontend?
...
1. `schema.sql`: explain the three tables.
...
| Why a dictionary? | Product ID to quantity; average constant-time lookup. |
